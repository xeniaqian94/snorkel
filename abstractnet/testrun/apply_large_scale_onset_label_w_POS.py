...
Example usage:
python apply_large_scale_onset_label_w_POS.py --train_bucket 0 --docs_per_bucket 100
python apply_large_scale_onset_label_w_POS.py --train_bucket 41 --docs_per_bucket 100
'''
...
parser.add_argument('--train_bucket', type=int, required=True)
parser.add_argument('--start_split', type=int, default=0)
parser.add_argument('--docs_per_bucket', type=int, default=300)
...
print("total LF count", len(purpose_LFs + mechanism_LFs + null_LFs), "unique count",
      len(set(purpose_LFs + mechanism_LFs + null_LFs)), "purpose_LFs", len(purpose_LFs), "mechanism_LFs",
      len(mechanism_LFs))
...
def get_L_train(LFs, parallelism=2, split=0):
    ...
    L_train = labeler.apply(
        split=split)  # ,cids_query=session.query(Candidate.id).filter(Candidate.get_parent().id %10==1))
    print(datetime.datetime.now())
    print(type(L_train))
    print(L_train.shape)
...
def get_candidate_text(segment, char_start, char_end_plus_1):
    return segment.get_parent().text[char_start:char_end_plus_1]


def get_candidate_pos(segment, word_idx):
    return segment.get_parent().__dict__['pos_tags'][word_idx]


def get_candidate_text_idx(segment, word_idx):
    return segment.get_parent().__dict__['words'][word_idx]


def get_candidate_dep(segment, word_idx):
    return segment.get_parent().__dict__['dep_parents'][word_idx]


def get_candidate_dep_labels(segment, word_idx):
    return segment.get_parent().__dict__['dep_labels'][word_idx]


def write_csv_from_L_train(L_train, csv_path, session):
    '''
    This function writes token, pos tag, dep number into the csv file
    '''
    try:
        ...
        for idx, train_segment in enumerate(unigram_segments_tmp):
            ...
            char_start = train_segment.get_contexts()[0].char_start
            char_end_plus_1 = train_segment.get_contexts()[0].char_end + 1
            word_idx = train_segment.get_contexts()[0].get_word_start()
            dep_parent = get_candidate_dep(train_segment, word_idx)
            pos=get_candidate_pos(train_segment, word_idx)
            text=get_candidate_text(train_segment, char_start, char_end_plus_1)
            aggregated_sent[sent_id] += [
                (cue.char_start, cue.char_end, text,
                 winningHighlight, pos,
                 dep_parent, word_idx, (get_candidate_pos(train_segment, dep_parent - 1) if dep_parent!=0 else pos),
                 (get_candidate_text_idx(train_segment, dep_parent - 1) if dep_parent!=0 else text), get_candidate_dep_labels(train_segment, word_idx))]
        ...
        for doc_id in aggregated_doc:
            ...
            for sent in aggregated_doc[doc_id]:
                ...
                for idx, pair in enumerate(aggregated_sent[sent_id]):
                    ...
                    df_list += [[pair[3], doc_id, globalPsn, text, pos, dep, sentPos, parPos, parText, depLabel]]
                    globalPsn += 1

        df = pd.DataFrame(df_list, columns=['winningHighlight', 'paperID', 'globalPsn', 'content', 'pos', 'dep',
                                            'sentPos', 'parPos', 'parText', 'depLabel'])
...
def multi_thread_train_label(split):
    ...
    write_csv_from_L_train(L_train_tmp,
                           "data/annotations_2k/largest-s2/onset_split_gz_" + str(train_bucket) + "_" + str(
                               split) + "_window" + str(smooth_window) + ".csv", session)
    print("write_csv_from_L_train finished")


for i in range(args.start_split, len(sents_split)):
    try:
