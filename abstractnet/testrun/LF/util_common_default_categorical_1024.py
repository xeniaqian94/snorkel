...
def purpose_for_doing(c):
    ...
    ing_idx=text_before_c.index(connection_word)+1

    if "ing"==text_before_c[ing_idx][-3:]:
...
def purpose_but_followed_next_sentence_we(c):
    c_idx=c.unigram_cue.get_word_start()

    text_whole_sent=get_surrounding_words(c,0,including_itself=True)

    if not ("but" in text_whole_sent or "however" in text_whole_sent):
        return 0

    sentence=c.get_parent()
    doc_sentences=c.get_parent().get_parent().sentences
    sent_idx=doc_sentences.index(sentence)
    if sent_idx==len(doc_sentences)-1:
        return 0
    next_sent_token=doc_sentences[sent_idx+1].__dict__['text']
